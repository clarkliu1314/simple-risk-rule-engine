 ...
 *
 */
public class LessEqualConditionalOperator implements ConditionalOperator {
	...
	public boolean operate(Object... objs) {
		...
		if (result.getClass().equals(Date.class)) {
			return ((Date)result).before((Date)setValue) || ((Date)result).equals((Date)setValue);
		}
		throw new IllegalArgumentException("参数错误，不能进行小于等于比较。" + result.toString());
	}
